...
def update_video_log_end(log_id, output_path, object_count, video_link, end_pressed_at_iso, error_msg=None):
    """
    Update the same row on END, set updatedAt to END pressed timestamp,
    and optionally save an error message.
    """
    try:
        with _get_conn() as conn:
            with conn.cursor() as cursor:
